...
public class BcastCollective {
  ...
  public static void main(String args[])
    ...
    Server server =
      new Server(host, port, eventQueue, dataMap,
        workers);
    server.start();
  ...
  static DoubleArray generateDoubleArray(
    int numBytes) {
  ...
  public static boolean runByteArrayChainBcast(
    ...
    for (int i = 0; i < numLoops; i++) {
      ...
      // Wait for ACK
      waitForACK(contextName, "byte-array-ack-"
        + i, workers.getMasterID(), dataMap,
        workers);
  ...
  private static void
    waitForACK(String contextName,
      String operationName, int bcastID,
    ...
    if (bcastID == workers.getSelfID()) {
      ...
      Data ackData =
        new Data(DataType.SIMPLE_LIST,
          ...
          operationName);
      Sender sender =
        new DataSender(ackData, bcastID, workers,
    ...
    for (int i = 0; i < numLoops; i++) {
      ...
      if (!workers.isMaster()) {
        for (Partition<DoubleArray> partition : arrTable
          ...
          LOG
            .info("Receive Double Array: first: "
              + doubleArray.get()[start]
              + ", last: "
              + doubleArray.get()[start + size
                - 1]);
        }
        arrTable.release();
      }
  ...
  public static <P extends Simple> boolean
    ...
    if (workers.getSelfID() == bcastWorkerID) {
      ...
      if (useMSTBcast) {
        sender =
          new DataMSTBcastSender(sendData,
